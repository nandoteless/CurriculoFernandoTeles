...
function exibirImagens() {
  ...
  // Verificação de segurança: só roda se o container existir (evita erro na página do jogo)
  if (!container) return; 
  ...
  for (let i = 0; i < imagensPorTela; i++) {
    ...
    img.onclick = function () {
      ...
      // --- ALTERAÇÃO: NAVEGAÇÃO DIRETA ---
      if (imagemInfo.link) {
         // Abre na mesma aba (navegação full screen)
         window.location.href = imagemInfo.link;
      }
      // Se não tiver link, abre o popup normal
      else {
            ...
            <html>
              <head>
                ...
                <style>
                  ...
                  img { width: ${widthStyle}; height: ${heightStyle}; border-radius: 8px; display: block; margin: 0 auto 15px auto; max-width: 100%; }
                  h1 { margin-top: 0; }
...
function mudarImagens(direcao) {  
  indiceAtual = (indiceAtual + direcao * 1 + imagens.length) % imagens.length;
  exibirImagens();
}
...
window.onload = function () {
  ...
  // Tenta exibir imagens, mas a função agora tem proteção interna se o elemento não existir
  exibirImagens();
};